...
import { glPostExpensePayment, glPostDeliveryWalletSettlement } from '@/lib/ledger/glBridge';
...
interface AccountingContextType {
  ...
  updateDeliveryOrderStatus: (orderId: string, status: DeliveryOrderStatus) => Promise<void>;
  collectCashFromDelivery: (deliveryPersonId: string, orderIds: string[], destination?: 'cash' | 'bank', notes?: string) => Promise<string>;
...
export const AccountingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateDeliveryOrderStatus = async (orderId: string, status: DeliveryOrderStatus) => {
  ...
  const collectCashFromDelivery = async (
    deliveryPersonId: string, 
    orderIds: string[], 
    ...
    const ordersToCollect = deliveryOrders.filter(o => orderIds.includes(o.id) && o.status === 'paid' && !o.returnedAt);
    const totalAmount = ordersToCollect.reduce((sum, o) => sum + o.amount, 0);

    if (totalAmount <= 0) throw new Error('No settled orders selected');
    if (totalAmount > person.walletBalance) {
      throw new Error('Settlement amount exceeds wallet balance');
    }

    const now = new Date().toISOString();
    const collectionId = `DWST-${Date.now()}`;

    if (storeId) {
      await glPostDeliveryWalletSettlement(storeId, collectionId, totalAmount, now, destination);
    }
    ...
    setCashBalance(prev => ({
      ...
      ...(destination === 'bank'
        ? { bankBalance: prev.bankBalance + totalAmount }
    ...
    const transaction: CashTransaction = {
      ...
      description: `Wallet settlement from ${person.name}`,
      createdAt: now
    };
    setCashTransactions(prev => [...prev, transaction]);

    console.log('[Accounting] Delivery wallet settled:', collectionId);
    return collectionId;
  };
