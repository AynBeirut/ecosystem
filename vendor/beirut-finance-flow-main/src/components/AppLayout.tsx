...
const PLATFORM_HOSTS = new Set(['localhost', '127.0.0.1', 'grabio.space', 'www.grabio.space']);

function isGrabioPlatformHost(): boolean {
  if (typeof window === 'undefined') return false;
  const host = window.location.hostname;
  if (PLATFORM_HOSTS.has(host)) return true;
  return /^market-flow-7b074--[a-z0-9-]+\.web\.app$/i.test(host);
}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className={cn(
        ...
        <div className="p-4 flex-shrink-0">
          ...
          {showGrabioAdminLink && (
            <a
              href={`${BRAND.ecosystemUrl}/admin/finance/invoices`}
              className="mt-3 flex items-center gap-2 rounded-lg border border-teal-200 bg-white px-3 py-2 text-sm font-medium text-teal-800 hover:bg-teal-50 dark:border-teal-800 dark:bg-teal-950/40 dark:text-teal-100 dark:hover:bg-teal-950/60"
            >
              <ArrowLeft className="h-4 w-4 shrink-0" />
              Back to Grabio Admin
            </a>
          )}
        ...
        <div className="flex-grow overflow-y-auto py-4 px-4 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700">
          <div className="space-y-1">
            ...
            <Button 
              variant="ghost" 
              className={cn("w-full justify-start",
                location.pathname === "/accounting" && "bg-gray-100 dark:bg-gray-800")}
              asChild
            >
              <Link to="/accounting">
                <BookOpen className="mr-2 h-4 w-4" /> Accounting
              </Link>
            </Button>
