...
const Inventory = () => {
  ...
  // Calculate stock value including composed products (platform FG cost when available)
  const totalStockValue = [...productItems, ...composedItems].reduce((sum, p) => {
    const cost = resolveDisplayUnitCost(p, calculateComposedProductCost);
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filteredProducts.map(product => {
                          const cost = resolveDisplayUnitCost(product, calculateComposedProductCost);
                          return (
                ...
                {composedItems.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {composedItems.map(product => {
                      const totalCost = resolveDisplayUnitCost(product, calculateComposedProductCost);
