...
export async function fsListCustomers<T extends DocumentData>(storeId: string): Promise<T[]> {
  const snap = await getDocs(query(collection(getFinanceDb(), 'customers'), where('storeId', '==', storeId)));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }) as T);
}

export async function fsListProducts<T extends DocumentData>(storeId: string): Promise<T[]> {
  const snap = await getDocs(query(collection(getFinanceDb(), 'products'), where('storeId', '==', storeId)));
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }) as T);
}
