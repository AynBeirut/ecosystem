import type { JournalLineInput } from '@/types/generalLedger';
...
export function buildCogsInventoryReliefLines(
  ...
  if (fgRelief > 0) {
    lines.push({ accountId: fgAccountId, debit: 0, credit: fgRelief, description: 'FG inventory relief' });
  }
  if (rawRelief > 0) {
    lines.push({ accountId: rawAccountId, debit: 0, credit: rawRelief, description: 'Raw materials relief' });
  }
  return lines;
}
...
export function parseCogsReliefSplit(
...
export function buildCogsInventoryReversalLines(
  ...
  if (split.fgRelief > 0) {
    lines.push({ accountId: fgAccountId, debit: split.fgRelief, credit: 0, description: 'Restore FG inventory' });
  }
  if (split.rawRelief > 0) {
    lines.push({ accountId: rawAccountId, debit: split.rawRelief, credit: 0, description: 'Restore raw materials' });
  }
  return lines;
}
