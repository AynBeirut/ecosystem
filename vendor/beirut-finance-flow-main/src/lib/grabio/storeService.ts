...
async function resolveStoreIdForUser(uid: string): Promise<string> {
  const userSnap = await getDoc(doc(getFinanceDb(), 'users', uid));
  if (userSnap.exists()) {
    const data = userSnap.data();
    if (data.role === 'sub_account' && data.subAccountId) {
      const subSnap = await getDoc(doc(getFinanceDb(), 'subAccounts', data.subAccountId));
  ...
  const sellerSnap = await getDoc(doc(getFinanceDb(), 'sellers', uid));
