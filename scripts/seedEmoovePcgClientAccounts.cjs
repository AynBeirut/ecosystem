#!/usr/bin/env node
/**
 * Bulk-import E-Moove client PCG codes ONLY (never Nipco / Little Hands).
 *
 *   node scripts/seedEmoovePcgClientAccounts.cjs --dry-run
 *   node scripts/seedEmoovePcgClientAccounts.cjs --apply
 *   node scripts/seedEmoovePcgClientAccounts.cjs --apply --file imports/emoove-pcg-client-accounts.csv
 */
const fs = require('fs');
const path = require('path');
const admin = require('firebase-admin');

const EMOOVE_STORE_ID = 'EZfuoNQFTJVU4cubNuckpp4K7zw2';
const DEFAULT_CSV = path.join(process.cwd(), 'imports/emoove-pcg-client-accounts.template.csv');
...
async function main() {
  ...
  if (!fs.existsSync(file)) {
    ...
    console.error('Fill ClientCode column in imports/emoove-pcg-client-accounts.template.csv');
    process.exit(1);
  }
  ...
  console.log(`Store: E-Moove only (${EMOOVE_STORE_ID})`);
  ...
  const col = db.collection('stores').doc(EMOOVE_STORE_ID).collection('pcgClientAccounts');
  ...
  await batch.commit();
  console.log(`\n✅ Imported ${rows.length} client account(s) for E-Moove.`);
}
