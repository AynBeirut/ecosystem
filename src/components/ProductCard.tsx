...
const ProductCard: React.FC<ProductCardProps> = ({ product, linkToStore, whatsappNumber, storeName, currency, displayType = 'grid-standard', animation = 'none' }) => {
  ...
  return (
    <Link to={cardLink}>
      <Card className={`h-full overflow-hidden card-hover ${displayType === 'masonry' ? 'border-0 shadow-sm bg-white/80' : ''} ${displayType === 'spotlight' ? 'ring-1 ring-market-primary/15' : ''} ${animationClass}`}>
