...
import {
  HOME_HERO,
  HOME_HERO_CRM_HREF,
  HOME_HERO_PACKAGE_SLUGS,
  HOME_HERO_RESTAURANT_DEMO_HREF,
  PHASE1_PACKAGES,
...
const HomeHero: React.FC = () => (
  <section className="marketing-hero">
    <div className="marketing-hero-inner">
      <p className="marketing-hero-eyebrow">Fine dining &amp; full service</p>
      ...
      <div className="marketing-hero-pills">
        <Link to={HOME_HERO_RESTAURANT_DEMO_HREF} className="marketing-hero-pill marketing-hero-pill-accent">
          Fine dining demo
        </Link>
        {PHASE1_PACKAGES.filter((pkg) =>
          (HOME_HERO_PACKAGE_SLUGS as readonly string[]).includes(pkg.slug) && pkg.slug !== 'restaurant',
        ).map((pkg) => (
          <Link key={pkg.slug} to={`/demo/${pkg.slug}`} className="marketing-hero-pill">
            {pkg.label}
          </Link>
        ))}
        <Link to={HOME_HERO_CRM_HREF} className="marketing-hero-pill">
          Guest CRM
        </Link>
