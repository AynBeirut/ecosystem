...
export type PurchaseReceiveSplit = {
  /** TTC credited to AP — matches subledger total / totalCost. */
  apCredit: number;
  /** Net inventory (ex-VAT). */
  inventoryDebit: number;
  /** Recoverable input VAT. */
  inputVatDebit: number;
};

/** Line-sum ex-VAT (legacy receive logic). */
...
/**
 * Split purchase receive for GL: Dr Inventory (+ Dr Input VAT) / Cr AP at TTC.
 * Aligns with purchase subledger (totalCost) and payment amountPaid.
 */
