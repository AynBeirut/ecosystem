...
export async function syncOrderSaleGl(orderId: string, orderData: Record<string, unknown>): Promise<boolean> {
  ...
  const glOrder = {
    ...
    total: absoluteTotal,
    subtotal: Math.round((Math.abs(Number(orderData.subtotal) || 0) + Number.EPSILON) * 100) / 100,
    discountAmount: Math.round(
      (Math.abs(Number(orderData.discountAmount ?? orderData.discount) || 0) + Number.EPSILON) * 100,
    ) / 100,
    ...
    clientId: normalizeString(orderData.clientId || orderData.customerId),
    clientName: normalizeString(orderData.clientName || orderData.customerName),
    customerId: normalizeString(orderData.customerId || orderData.clientId),
    customerName: normalizeString(orderData.customerName || orderData.clientName),
    cogsLines,
