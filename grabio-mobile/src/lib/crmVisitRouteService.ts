...
import { logActivity } from './crmMobileService';
import type { CrmClient } from './crmMobileService';
import { applyRouteVisitScheduleToCustomers } from './crmVisitScheduleService';
...
/** Whether a repeating route applies on the given calendar day. */
...
export function routeProgressOnDate(route: CrmVisitRoute, occurrenceYmd: string): { done: number; total: number } {
...
export async function updateVisitRoute(input: {
  ...
  await firestore().collection('crmVisitRoutes').doc(input.routeId).update({
...
export async function createVisitRoute(input: {
  ...
  const now = new Date().toISOString();
  const ref = await firestore().collection('crmVisitRoutes').add({
    storeId: input.storeId,
    title: input.title.trim() || `Visit route · ${input.visitDate}`,
    assignedRepId: input.assignedRepId,
    assignedRepName: input.assignedRepName,
...
export async function fetchVisitRoutes(
  ...
  try {
    if (managerView) {
      const snap = await firestore().collection('crmVisitRoutes').where('storeId', '==', storeId).get();
...
/** Match route assignment against one id or any alias id for the signed-in rep. */
...
export async function fetchVisitRoute(routeId: string): Promise<CrmVisitRoute | null> {
  try {
    const snap = await firestore().collection('crmVisitRoutes').doc(routeId).get();
...
/** Today ± window — includes recent past routes still open (yesterday's visit, etc.). */
...
/** One row per route — nearest upcoming day only (avoids weekly duplicates). */
...
import type { CrmActivityResult } from './crmConstants';
...
export async function markVisitRouteStopDone(input: {
  ...
  await firestore().collection('crmVisitRoutes').doc(route.id).update({
