...
const ADMIN_APP_SESSION_KEY = 'grabio-admin-app-shell';
const PLAY_STORE_PACKAGE = 'space.grabio.finance';
const GRABIO_ADMIN_PACKAGE = 'space.grabio.app';
...
/** Call once on app boot — marks Play Store / TWA / Grabio Admin WebView sessions. */
function markPlayStoreShell(): void {
  try {
...
export function bootstrapPlayStoreShell(): void {
  ...
  const fromLaunchParam =
    params.get('source') === 'grabio-finance-app' ||
    params.get('source') === 'grabio-admin-app' ||
    params.get('app') === 'play' ||
    params.get('embedded') === '1';
  const fromReferrer =
    document.referrer.startsWith(`android-app://${PLAY_STORE_PACKAGE}`) ||
    document.referrer.startsWith(`android-app://${GRABIO_ADMIN_PACKAGE}`);
  const fromAndroidInvoicePath =
    isAndroidWebView() &&
    window.location.hostname === 'grabio.space' &&
    (window.location.pathname === '/invoice' || window.location.pathname.startsWith('/invoice/'));
  const fromStandaloneDisplay =
    window.matchMedia('(display-mode: standalone)').matches ||
    window.matchMedia('(display-mode: fullscreen)').matches ||
    (window.navigator as Navigator & { standalone?: boolean }).standalone === true;

  if (fromLaunchParam || fromReferrer || fromAndroidInvoicePath || fromStandaloneDisplay) {
    markPlayStoreShell();
    if (params.get('source') === 'grabio-admin-app' || params.get('embedded') === '1') {
      try {
        sessionStorage.setItem(ADMIN_APP_SESSION_KEY, '1');
        localStorage.setItem(ADMIN_APP_SESSION_KEY, '1');
      } catch {
        /* private mode */
      }
    }
...
export function isPlayStoreV1Shell(): boolean {
  ...
  if (document.referrer.startsWith(`android-app://${GRABIO_ADMIN_PACKAGE}`)) return true;

  try {
    if (sessionStorage.getItem(ADMIN_APP_SESSION_KEY) === '1') return true;
    if (localStorage.getItem(ADMIN_APP_SESSION_KEY) === '1') return true;
  } catch {
    /* ignore */
  }

  // TWA / WebView on grabio.space/invoice — treat as app shell even when display-mode is browser
...
/** True when opened inside Grabio Admin mobile WebView. */
export function isGrabioAdminAppShell(): boolean {
  if (typeof window === 'undefined') return false;
  try {
    return (
      sessionStorage.getItem(ADMIN_APP_SESSION_KEY) === '1' ||
      localStorage.getItem(ADMIN_APP_SESSION_KEY) === '1'
    );
  } catch {
    return false;
  }
...
/** Standalone Invoice Manager / TWA / Grabio Admin WebView — no external browser links. */
export function isFinanceInAppShell(): boolean {
  return isPlayStoreV1Shell() || isGrabioAdminAppShell();
}
