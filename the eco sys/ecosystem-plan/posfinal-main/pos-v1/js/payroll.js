...
async function getStaffPaymentHistory(staffId, filters = {}) {
    try {
        ...
        // Filter out attendance entries already covered by a pending/approved payment
        // (prevents double-counting: salary payment + individual attendance rows for the same period)
        if (payments.length > 0 && unpaidAttendance.length > 0) {
            const coveredRanges = payments
                .filter(p => p.status !== 'paid')
                .map(p => ({
                    start: new Date(p.periodStart).toISOString().split('T')[0],
                    end: new Date(p.periodEnd).toISOString().split('T')[0]
                }));
            if (coveredRanges.length > 0) {
                unpaidAttendance = unpaidAttendance.filter(att =>
                    !coveredRanges.some(range =>
                        att.attendanceDate >= range.start && att.attendanceDate <= range.end
                    )
                );
            }
        }
