...
// Silently seeds the DB from default-data.sqlite when storage is empty.
// ===================================
async function autoLoadDefaultData() {
    try {
        console.log('🔄 DB empty — fetching default-data.sqlite...');
        const resp = await fetch('./default-data.sqlite');
        if (!resp.ok) {
            console.log('ℹ️ default-data.sqlite not found, starting fresh.');
            return false;
        }
        const buf = await resp.arrayBuffer();
        const uint8 = new Uint8Array(buf);
        const tempDb = new SQL.Database(uint8);
        const products = tempDb.exec('SELECT COUNT(*) FROM products')[0]?.values[0][0] || 0;
        const sales    = tempDb.exec('SELECT COUNT(*) FROM sales')[0]?.values[0][0]    || 0;
        if (products === 0 && sales === 0) {
            tempDb.close();
            console.warn('⚠️ default-data.sqlite is also empty — ignoring.');
            return false;
        }
        db = tempDb;
        console.log(`✅ Default data loaded: ${products} products, ${sales} sales`);
        return true;
    } catch (e) {
        console.warn('⚠️ autoLoadDefaultData failed:', e.message);
        return false;
    }
}
    ...
    try {
        ...
        if (existingDb && existingDb.length > 0) {
            ...
            try {
                ...
                // If current DB looks empty, auto-load default data first (silent, no prompts).
                if (productCount === 0 && salesCount === 0) {
                    const defaultLoaded = await autoLoadDefaultData();
                    if (!defaultLoaded) {
            ...
            console.log('⚠️ NO EXISTING DATABASE FOUND - trying default data...');
            const defaultLoaded = await autoLoadDefaultData();
            if (!defaultLoaded) {
                console.log('ℹ️ No default data, creating empty database');
                db = new SQL.Database();
            } else {
                loadedExistingDatabase = true;
            }
