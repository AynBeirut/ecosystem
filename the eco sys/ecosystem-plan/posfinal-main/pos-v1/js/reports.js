...
function getNormalizedSaleTotals(parsedSale, fallbackSubtotal = 0) {
    const saleTotals = (parsedSale?.totals && typeof parsedSale.totals === 'object') ? parsedSale.totals : {};

    const subtotal = toFiniteNumber(
        saleTotals.subtotal,
        toFiniteNumber(parsedSale?.subtotal, fallbackSubtotal)
    );

    const discountPercent = toFiniteNumber(saleTotals.discountPercent, toFiniteNumber(parsedSale?.discountPercent, 0));
    const discount = toFiniteNumber(
        saleTotals.discount,
        toFiniteNumber(parsedSale?.discount, subtotal * (discountPercent / 100))
    );
    const tax = toFiniteNumber(saleTotals.tax, toFiniteNumber(parsedSale?.tax, 0));

    const totalFallback = Math.max(0, subtotal - discount) + tax;
    const total = toFiniteNumber(
        saleTotals.total,
        toFiniteNumber(parsedSale?.total, totalFallback)
    );

    const resolvedDiscountPercent = discountPercent > 0
        ? discountPercent
        : (subtotal > 0 && discount > 0 ? (discount / subtotal) * 100 : 0);
    ...
    return {
        ...
        tax,
        total,
        taxEnabled: saleTotals.taxEnabled !== undefined ? !!saleTotals.taxEnabled : tax > 0
    };
}

function getSaleRevenueForReports(parsedSale, itemMetrics) {
    const grossRevenue = itemMetrics.revenue;

    // Product-level filtered reports show product gross revenue only.
    if (activeFilters?.productId) {
        return grossRevenue;
    }

    const normalizedTotals = getNormalizedSaleTotals(parsedSale, grossRevenue);
    const boundedDiscount = Math.min(Math.max(0, normalizedTotals.discount), Math.max(0, normalizedTotals.subtotal));
    return Math.max(0, normalizedTotals.subtotal - boundedDiscount);
}
...
function calculateStats(sales) {
    ...
    const totalItems = reportSales.reduce((sum, sale) => {
        ...
        const saleRevenue = getSaleRevenueForReports(parsed, metrics);
        ...
        <table class="sales-table-grid">
            ...
            <tbody>
                ${recentSales.map((sale, index) => {
                    ...
                    const normalizedTotals = getNormalizedSaleTotals(parsed, metrics.revenue);
                    const totalRevenue = getSaleRevenueForReports(parsed, metrics);
                    ...
                    let taxDiscountInfo = '';
                    if (totals || normalizedTotals) {
                        if ((normalizedTotals.discount || 0) > 0 || (normalizedTotals.discountPercent || 0) > 0) {
                            taxDiscountInfo += `<div class="sale-item-detail" style="background: #fef3c7; padding: 4px 8px; margin-top: 4px;">
                                <span style="color: #92400e;">💰 Discount: ${normalizedTotals.discountPercent.toFixed(2)}% ($${normalizedTotals.discount.toFixed(2)})</span>
                            </div>`;
                        }
                        if (normalizedTotals.taxEnabled && normalizedTotals.tax > 0) {
                            taxDiscountInfo += `<div class="sale-item-detail" style="background: #dbeafe; padding: 4px 8px; margin-top: 4px;">
                                <span style="color: #1e40af;">🧾 Tax (11%): $${normalizedTotals.tax.toFixed(2)}</span>
                            </div>`;
                        }
                    }
