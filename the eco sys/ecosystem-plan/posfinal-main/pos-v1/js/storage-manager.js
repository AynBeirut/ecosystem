...
const BROWSER_STORAGE_PREF_KEY = `${GLOBAL_DB_KEY}_browser_storage_preference`;
const BROWSER_STORAGE_PREF_INDEXEDDB = 'indexeddb';
const BROWSER_STORAGE_PREF_FILE_SYSTEM = 'file-system-api';
...
function detectPlatform() {
    ...
    // Browser mode defaults to IndexedDB for zero-prompt startup.
    const browserStoragePreference = localStorage.getItem(BROWSER_STORAGE_PREF_KEY) || BROWSER_STORAGE_PREF_INDEXEDDB;
    
    // Web browser fallbacks
    if ('showSaveFilePicker' in window && browserStoragePreference === BROWSER_STORAGE_PREF_FILE_SYSTEM) {
        console.log('💾 Using File System Access API (user preference)');
...
async function detectStorageType() {
    // Keep one source of truth for storage detection.
...
async function fileSystemApiSave(dbName, data) {
    try {
        ...
        if (error.name === 'AbortError') {
            ...
            localStorage.setItem(BROWSER_STORAGE_PREF_KEY, BROWSER_STORAGE_PREF_INDEXEDDB);
            return indexedDbSave(dbName, data);
        }

        if (error.name === 'SecurityError') {
            console.warn('File System API requires user gesture; switching to IndexedDB');
            currentStorageType = STORAGE_TYPES.INDEXEDDB;
            localStorage.setItem(BROWSER_STORAGE_PREF_KEY, BROWSER_STORAGE_PREF_INDEXEDDB);
            return indexedDbSave(dbName, data);
        }
...
if (typeof window !== 'undefined') {
    ...
    window.setBrowserStoragePreference = (preference) => {
        if (preference === BROWSER_STORAGE_PREF_FILE_SYSTEM || preference === BROWSER_STORAGE_PREF_INDEXEDDB) {
            localStorage.setItem(BROWSER_STORAGE_PREF_KEY, preference);
            currentStorageType = null; // Re-detect on next operation
            return true;
        }
        return false;
    };
